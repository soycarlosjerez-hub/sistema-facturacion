...
{
    private function setFkChecks(bool $on): void
    {
        // MySQL-only; en SQLite/Postgres se omite (no soportan SET FOREIGN_KEY_CHECKS).
        if (DB::getDriverName() !== 'mysql') {
            return;
        }
        DB::statement($on ? 'SET FOREIGN_KEY_CHECKS=1' : 'SET FOREIGN_KEY_CHECKS=0');
    }

    public function up(): void
    {
        $this->setFkChecks(false);

        // Phase 1: Add columns to categorias
        Schema::table('categorias', function (Blueprint $table) {
            if (! Schema::hasColumn('categorias', 'color')) {
                $table->string('color', 7)->default('#6366f1')->after('activa');
            }
            if (! Schema::hasColumn('categorias', 'icono')) {
                $table->string('icono', 50)->default('bi-grid')->after('color');
            }
            if (! Schema::hasColumn('categorias', 'orden')) {
                $table->integer('orden')->default(0)->after('icono');
            }
            if (! Schema::hasColumn('categorias', 'configuracion')) {
                $table->json('configuracion')->nullable()->after('orden');
            }
            if (! Schema::hasColumn('categorias', 'deleted_at')) {
                $table->timestamp('deleted_at')->nullable()->after('configuracion');
            }
        });

        // Phase 2: Drop old tables
        try {
            DB::statement('DROP TABLE IF EXISTS categorizables');
        } catch (QueryException $e) {
        }
        try {
            DB::statement('DROP TABLE IF EXISTS category_subcategories');
        } catch (QueryException $e) {
        }
        try {
