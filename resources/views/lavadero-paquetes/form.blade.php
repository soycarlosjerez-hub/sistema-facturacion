...
@section('title', $paquete->exists ? 'Editar Paquete' : 'Nuevo Paquete')
    ...
    <div class="ui-header mb-4">
        ...
        <div class="ui-header-body">
            <div class="ui-header-left">
                ...
                <div>
                    <h4 class="ui-header-title">{{ $paquete->exists ? 'Editar Paquete: ' . $paquete->nombre : 'Nuevo Paquete' }}</h4>
                ...
                <div class="ui-card">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="nombre" class="ui-input" value="{{ old('nombre', $paquete->nombre) }}" required>
                        </div>
                        <div class="mb-3">
                            <label class="ui-label">Descripción</label>
                            <textarea name="descripcion" class="ui-textarea" rows="2">{{ old('descripcion', $paquete->descripcion) }}</textarea>
                        </div>
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="ui-label">Tipo de Vehículo</label>
                                <select name="aplicable_a_tipo" class="ui-select">
                                    <option value="todos" {{ old('aplicable_a_tipo', $paquete->aplicable_a_tipo) == 'todos' ? 'selected' : '' }}>Todos</option>
                                    <option value="sedan" {{ old('aplicable_a_tipo', $paquete->aplicable_a_tipo) == 'sedan' ? 'selected' : '' }}>Sedán / Auto Pequeño</option>
                                    <option value="suv" {{ old('aplicable_a_tipo', $paquete->aplicable_a_tipo) == 'suv' ? 'selected' : '' }}>SUV</option>
                                    <option value="pickup" {{ old('aplicable_a_tipo', $paquete->aplicable_a_tipo) == 'pickup' ? 'selected' : '' }}>Pickup</option>
                                    <option value="van" {{ old('aplicable_a_tipo', $paquete->aplicable_a_tipo) == 'van' ? 'selected' : '' }}>Van / Minivan</option>
                                    <option value="moto" {{ old('aplicable_a_tipo', $paquete->aplicable_a_tipo) == 'moto' ? 'selected' : '' }}>Moto / Scooter</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="ui-label">Duración (minutos)</label>
                                <input type="number" name="duracion_minutos" class="ui-input" value="{{ old('duracion_minutos', $paquete->duracion_minutos) }}" min="5">
                            </div>
                        </div>
                        <div class="mt-3">
                        ...
                        <div id="paquete-items-container">
                            @forelse($paquete->items ?? [] as $item)
                            <div class="paquete-item-row">
                                <div class="row g-2 align-items-center">
                                    <div class="col-md-3">
                                        <label class="ui-label small">Tipo</label>
                                        <select name="items[{{ $loop->index }}][tipo]" class="ui-select ui-select-sm">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select name="items[{{ $loop->index }}][servicio_id]" class="form-select form-select-sm">
                                            <option value="">Seleccionar...</option>
                                            @foreach($servicios as $s)
                                            <option value="{{ $s->id }}" {{ $item->servicio_id == $s->id ? 'selected' : '' }}>{{ $s->nombre }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                                        <label class="ui-label small">Cantidad</label>
                                        <input type="number" name="items[{{ $loop->index }}][cantidad]" class="ui-input ui-input-sm" value="{{ $item->cantidad }}" min="1" step="0.5">
                                    </div>
                                    <div class="col-md-2">
                                        <label class="ui-label small">Precio Ind.</label>
                                        <input type="number" name="items[{{ $loop->index }}][precio_individual]" class="ui-input ui-input-sm" value="{{ $item->precio_individual ?? '' }}" step="0.01">
                                    </div>
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="ui-input-group">
                                ...
                                <input type="number" name="precio" id="paquete-precio" class="ui-input" value="{{ old('precio', $paquete->precio) }}" step="0.01" oninput="calcTotal()">
                            </div>
                        </div>
                        <div class="mb-3">
                            ...
                            <div class="ui-input-group">
                                <span class="ui-input-group-text">RD$</span>
                                <input type="number" name="precio_anterior" class="ui-input" value="{{ old('precio_anterior', $paquete->precio_anterior) }}" step="0.01">
                            </div>
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <input type="text" name="tags[]" class="ui-input mt-2" value="{{ old('tags', is_array($paquete->tags) ? implode(',', $paquete->tags) : '') }}" placeholder="premium, popular, nuevo">
...
<div class="ui-sticky-bar">
    <div class="ui-sticky-bar-inner">
        ...
        <button type="submit" form="paquete-form" class="ui-btn ui-btn-solid rounded-pill px-5">
            <i class="bi bi-check-lg me-2"></i>{{ $paquete->exists ? 'Actualizar' : 'Crear' }}
        </button>
    </div>
</div>
...
@section('scripts')
<script>
let itemCounter = {{ count($paquete->items ?? []) }};
function addItem() {
    ...
    const html = `<div class="paquete-item-row">
        <div class="row g-2 align-items-center">
            <div class="col-md-3">
                <label class="ui-label small">Tipo</label>
                <select name="items[${idx}][tipo]" class="ui-select ui-select-sm" onchange="toggleServicioField(this)">
            ...
            <div class="col-md-4">
                <label class="ui-label small">Servicio</label>
                <select name="items[${idx}][servicio_id]" class="form-select form-select-sm">
                    <option value="">Seleccionar...</option>
                    @foreach($servicios as $s)
...
function toggleServicioField(sel) {}
function calcTotal() {
    let total = 0;
