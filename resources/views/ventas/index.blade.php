    ...
    <div class="premium-header d-flex justify-content-between align-items-center">
        <div>
            <h2 class="fw-bold mb-1 d-flex align-items-center">
                <i class="bi bi-receipt me-3 fs-1 opacity-75"></i>
                Gestión de Ventas
            </h2>
        ...
        <form method="GET" class="row g-3 align-items-end" id="filtros-form">
            <div class="col-lg-4">
                ...
                <div class="input-group">
                    ...
                    <input type="text" name="cliente" id="busqueda-cliente" class="form-control border-start-0 ps-0" 
                           placeholder="Buscar por cliente o NCF..." value="{{ request('cliente') }}" autocomplete="off">
                </div>
            </div>
            <div class="col-lg-2">
            ...
            <div class="col-lg-2 text-end">
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end border-0 shadow-lg rounded-3">
                        <li><a class="dropdown-item" href="{{ route('ventas.pdf', request()->query()) }}"><i class="bi bi-file-pdf text-danger me-2"></i> Descargar PDF</a></li>
                        <li><a class="dropdown-item" href="{{ route('ventas.exportar', request()->query()) }}"><i class="bi bi-file-excel text-success me-2"></i> Exportar a Excel</a></li>
                    </ul>
                </div>
            </div>
        </form>
                ...
                <tr>
                    <td class="ps-4">
                        <div class="d-flex flex-column">
                            ...
                            @if($v->ncf)
                                <span class="status-badge bg-light text-muted mt-1 border d-inline-block text-truncate" style="max-width: 130px;">
                    ...
                    <td>
                        <span class="status-badge {{ $esFiado ? 'bg-warning bg-opacity-25 text-dark' : 'bg-info bg-opacity-10 text-info' }} d-inline-block">
                            @if($esFiado)
                    ...
                    <td class="text-center">
                        @if ($v->estado == 'completada')
                            <span class="status-badge bg-success bg-opacity-10 text-success d-inline-block">
                                <i class="bi bi-check-circle-fill me-1"></i>Pagada
                            </span>
                        @elseif ($v->estado == 'cuenta_abierta')
                            <span class="status-badge bg-primary bg-opacity-10 text-primary d-inline-block">
                                <i class="bi bi-door-open-fill me-1"></i>Cta. Abierta
                            </span>
                        @else
                            <span class="status-badge bg-warning bg-opacity-10 text-warning d-inline-block">
                    ...
                    <td class="text-end pe-4">
                        <div class="dropdown d-inline-block">
                            <button class="btn btn-icon-hover text-muted border-0 bg-transparent" data-bs-toggle="dropdown">
                                <i class="bi bi-three-dots-vertical"></i>
                            </button>
                            <ul class="dropdown-menu dropdown-menu-end border-0 shadow-lg rounded-3 py-2">
                                <li><a class="dropdown-item py-2" href="{{ route('ventas.show', $v->id) }}"><i class="bi bi-eye text-primary me-2"></i> Ver Detalles</a></li>
                                <li><a class="dropdown-item py-2" href="{{ route('pagos.realizar', $v->id) }}"><i class="bi bi-cash-coin text-success me-2"></i> Cobrar / Abono</a></li>
                                <li><a class="dropdown-item py-2" href="{{ route('venta.pdf', $v->id) }}"><i class="bi bi-printer text-muted me-2"></i> Reimprimir</a></li>
                                <li><hr class="dropdown-divider"></li>
                                <li>
                                    <form action="{{ route('ventas.destroy', $v->id) }}" method="POST" onsubmit="return confirm('¿Anular esta venta?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="dropdown-item py-2 text-danger"><i class="bi bi-x-circle me-2"></i> Anular Venta</button>
                                    </form>
                                </li>
                            </ul>
                        </div>
                ...
                <tr>
                    <td colspan="7" class="text-center py-5">
                        <div class="d-flex flex-column align-items-center justify-content-center p-5">
                            ...
                            <p class="text-muted mb-4 text-center" style="max-width: 400px;">Aún no se han registrado ventas con estos filtros. Puedes crear tu primera venta ahora.</p>
                            @can('ventas.create')
                            <a href="{{ route('ventas.create') }}" class="btn btn-primary rounded-pill px-4 py-2 shadow-sm">
                                <i class="bi bi-plus-lg me-2"></i> Registrar Nueva Venta
                            </a>
                            @endcan
                        </div>
                    </td>
