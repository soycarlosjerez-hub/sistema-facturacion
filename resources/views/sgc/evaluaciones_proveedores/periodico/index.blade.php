...
@section('title', 'Evaluaciones Periódicas de Proveedores')
            ...
            <div class="ui-header-left">
                ...
                <div>
                    ...
                    <div class="ui-header-meta">
                        ...
                        <a href="{{ route('sgc.evaluaciones-proveedores.index') }}" class="text-white text-decoration-none">Evaluaciones</a>
                        <span class="mx-1">/</span> Periódicas
                    </div>
                </div>
            </div>
            <div class="ui-header-actions">
                <a href="{{ route('sgc.evaluaciones-proveedores.periodico.create') }}" class="ui-btn ui-btn-primary ui-btn-sm rounded-pill">
    ...
    <div class="ui-card" style="--delay:.1s">
        ...
        <div class="ui-card-body p-0">
            <table class="dt-table datatable" id="periodico-table">
                ...
                <tbody>
                    @foreach($evaluacionesPeriodicas as $eval)
                    <tr>
                        <td><code>{{ $eval->codigo }}</code></td>
                        <td class="fw-semibold">{{ $eval->proveedor?->nombre ?? '—' }}</td>
                        <td>{{ $eval->periodo ?? '—' }}</td>
                        <td>{{ $eval->fecha_inicio ? $eval->fecha_inicio->format('d/m/Y') : '—' }}</td>
                        <td>{{ $eval->fecha_fin ? $eval->fecha_fin->format('d/m/Y') : '—' }}</td>
                        <td>
                            @if($eval->puntuacion)
                        ...
                        <td>
                            <div class="d-flex gap-1">
                                <a href="{{ route('sgc.evaluaciones-proveedores.periodico.show', $eval) }}" class="btn btn-sm btn-outline-primary rounded-pill" title="Ver">
                                    <i class="bi bi-eye"></i>
                                </a>
                            </div>
                        </td>
