...
<div class="container-fluid px-4 py-4">
    ...
    <div class="premium-header d-flex justify-content-between align-items-center">
        <div>
            <h2 class="fw-bold mb-1 d-flex align-items-center">
                <i class="bi bi-people me-3 fs-1 opacity-75"></i>
        ...
        <form method="GET" id="search-form" class="row g-3 align-items-end">
            <div class="col-lg-6">
                ...
                <div class="input-group">
                    ...
                    <input type="text" name="nombre" id="busqueda-cliente" class="form-control border-start-0 ps-0" 
                           placeholder="Nombre, RNC/Cédula, teléfono o email..." value="{{ request('nombre') }}" autocomplete="off">
                </div>
            </div>
    ...
    <!-- Lista de Clientes -->
    <div class="table-responsive" style="min-height: 400px;" id="clientes-container">
        <table class="table table-custom mb-0 w-100">
            <thead class="text-muted small text-uppercase tracking-wider" style="border-bottom: 2px solid #e2e8f0;">
                <tr>
                    ...
                    <th class="pb-3 text-center">Identificación</th>
                    <th class="pb-3 text-end">Balance Pendiente</th>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        @if($c->balance_pendiente > 5000)
                            <span class="status-badge bg-danger bg-opacity-10 text-danger shadow-sm d-inline-block">
                                <i class="bi bi-exclamation-triangle-fill me-1"></i> Riesgo Alto
                            </span>
                        @elseif($c->balance_pendiente > 0)
                            <span class="status-badge bg-warning bg-opacity-25 text-dark shadow-sm d-inline-block">
                    ...
                    <td class="text-end pe-4">
                        <div class="d-flex justify-content-end gap-1">
                            <a href="{{ route('clientes.show', $c) }}" class="btn btn-icon-hover text-info" title="Ver detalle completo">
                                <i class="bi bi-eye"></i>
                            </a>
                            <a href="{{ route('clientes.edit', $c) }}" class="btn btn-icon-hover text-primary" title="Editar datos">
                            ...
                            <form action="{{ route('clientes.destroy', $c) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Seguro que deseas eliminar este cliente? Se perderá su historial si no tiene ventas asociadas.')">
                                @csrf @method('DELETE')
                                <button type="submit" class="btn btn-icon-hover text-danger border-0 bg-transparent" title="Eliminar cliente">
                ...
                <tr>
                    <td colspan="6" class="text-center py-5">
                        <div class="d-flex flex-column align-items-center justify-content-center p-5">
                            ...
                            <p class="text-muted mb-4 text-center" style="max-width: 400px;">Aún no se han registrado clientes en tu directorio o no coinciden con tu búsqueda. ¡Empieza a crear tu cartera de clientes!</p>
