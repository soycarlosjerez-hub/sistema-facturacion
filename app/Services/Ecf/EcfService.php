...
class EcfService
{
    ...
    {
        try {
            $ecf = $ecf->fresh(['secuencia', 'venta.cliente', 'venta.detalles.producto', 'compra.proveedor', 'compra.detalles.producto']);
            $ecf->transicionarA('firmado');

            $xml = $ecf->tipo_ecf === 'E41' && $ecf->compra
                ? $this->builder->buildE41($ecf, $ecf->compra)
                : $this->builder->build($ecf);
    ...
    public function enviar(EcfDocumento $ecf): EcfDocumento
    {
        ...
        if ($response['success'] && ($response['estado'] ?? '') === 'aprobado') {
            ...
            $ecf->save();
            // Código de seguridad oficial del acuse (sustituye el provisional).
            $this->qr->actualizarDesdeAcuse($ecf->fresh(), $response);
            $ecf = $ecf->fresh();
        } else {
    ...
    public function generarE41(Compra $compra): EcfDocumento
    {
        ...
        try {
            ...
            Log::error('e-CF: E41 generada pero falló firma/envío (quedó pendiente de reintento)', [
    ...
    private function validarXml(string $xml, EcfDocumento $ecf): void
    {
        ...
        if (! file_exists($xsdPath)) {
            // En producción el XSD oficial DGII es obligatorio: sin él no hay
            // garantía de que el XML será aceptado. Subirlo a storage/app/ecf/schema/DGII_ecf.xsd.
            if (config('dgii.ambiente') === 'produccion' && ! config('dgii.simular_dgii')) {
                throw new \RuntimeException(
                    "Esquema XSD DGII no encontrado en {$xsdPath}. ".
                    'Suba el XSD oficial antes de emitir e-CF en producción.'
                );
            }
