<?php $__env->startSection('title', 'Backups del Sistema'); ?>
...
<div class="ui-page" style="--accent:#f59e0b;--accent-rgb:245,158,11;--accent-hover:#d97706;">
            ...
            <div class="ui-header-left">
                ...
                <div>
                    ...
                    <div class="ui-header-meta">
                        ...
                        <span><?php echo e($backups->total()); ?> respaldo(s)</span>
                    </div>
                </div>
            </div>
            <div class="ui-header-actions">
                <a href="<?php echo e(route('owner.backups.config')); ?>" class="ui-btn ui-btn-primary ui-btn-sm ui-btn-pill">
    ...
    <form id="backupForm" method="POST" action="<?php echo e(route('owner.backups.store')); ?>" style="display:none;">
        <?php echo csrf_field(); ?>
        ...
        <div class="col-6 col-lg-3">
            <div class="ui-stat" style="--delay:.05s">
                ...
                <div class="ui-stat-body text-center">
                    ...
                    <div class="ui-stat-value"><?php echo e($backups->total()); ?></div>
        ...
        <div class="col-6 col-lg-3">
            <div class="ui-stat" style="--delay:.15s">
                ...
                <div class="ui-stat-body text-center">
                    ...
                    <div class="ui-stat-value" style="color:#06b6d4 !important;"><?php echo e($last7Days); ?></div>
            ...
            <div class="ui-stat" style="--delay:.2s">
                ...
                <div class="ui-stat-body text-center">
                    ...
                    <div class="ui-stat-value" style="font-size:1rem;">
                        <?php echo e($lastBackup ? $lastBackup->created_at->diffForHumans() : 'N/A'); ?>

                    </div>
                    <div class="ui-stat-sub">
                        <?php echo e($lastBackup ? date('d/m/Y H:i', strtotime($lastBackup->created_at)) : 'Sin backups'); ?>
        ...
        <div class="ui-card-body p-0">
            <div class="table-responsive">
                <table class="table ui-table mb-0">
                    ...
                    <tbody>
                        <?php $__empty_0 = true; $__currentLoopData = $backups; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $backup): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_0 = false; ?>
                            <tr>
                                <td class="ps-4">
                                    <span class="fw-semibold small font-monospace"><?php echo e($backup->filename); ?></span>
                                    <?php if($backup->notes): ?>
                                        <br><small class="text-muted"><?php echo e($backup->notes); ?></small>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <?php if($backup->type === 'automatico'): ?>
                                        <span class="ui-badge ui-badge-info"><i class="bi bi-clock-history me-1"></i>Automático</span>
                                    <?php elseif($backup->type === 'restore'): ?>
                                ...
                                <td>
                                    <?php if($backup->status === 'completado'): ?>
                                ...
                                <td>
                                    <?php if($backup->size_bytes > 0): ?>
                                        <span class="fw-semibold"><?php echo e($backup->sizeForHumans()); ?></span>
                                    <?php else: ?>
                                    ...
                                    <?php if($backup->user): ?>
                                        <div class="d-flex align-items-center gap-2">
                                            <div class="d-flex align-items-center justify-content-center" style="width:28px;height:28px;border-radius:50%;background:rgba(245,158,11,.15);">
                                                <span class="fw-bold small" style="color:#f59e0b;"><?php echo e(strtoupper(substr($backup->user->name, 0, 1))); ?></span>
                                            </div>
                                            <small class="text-muted"><?php echo e($backup->user->name); ?></small>
                                        </div>
                                    <?php else: ?>
                                ...
                                <td>
                                    <small><?php echo e($backup->created_at ? date('d/m/Y H:i', strtotime($backup->created_at)) : 'N/A'); ?></small>
                                </td>
                                <td class="text-end pe-4">
                                    <div class="d-flex gap-1 justify-content-end">
                                        <?php if($backup->status === 'completado' && $backup->size_bytes > 0 && !str_starts_with($backup->filename, 'RESTORE:')): ?>
                                            <a href="<?php echo e(route('owner.backups.download', $backup)); ?>" class="ui-action ui-action-print" title="Descargar" data-bs-toggle="tooltip">
                                                <i class="bi bi-download"></i>
                                            </a>
                                            <button type="button" class="ui-action ui-action-delete"
                                                    onclick="UI.confirm.action({title:'Restaurar Backup', text:'Esto reemplazará TODOS los datos actuales. ¿Continuar?', icon:'warning', color:'#ef4444', confirmText:'Sí, Restaurar', onSubmit:function(){ document.getElementById('restoreForm_<?php echo e($backup->id); ?>').submit(); }})"
                                                    title="Restaurar" data-bs-toggle="tooltip">
                                                <i class="bi bi-arrow-repeat"></i>
                                            </button>
                                            <form id="restoreForm_<?php echo e($backup->id); ?>" action="<?php echo e(route('owner.backups.restore', $backup)); ?>" method="POST" style="display:none;">
                                                <?php echo csrf_field(); ?>
                                                <input type="hidden" name="confirm" value="1">
                                            </form>
                                        <?php endif; ?>
                                        <button type="button" class="ui-action ui-action-delete"
                                                onclick="UI.confirm.delete('<?php echo e(route('owner.backups.destroy', $backup)); ?>', '<?php echo e($backup->filename); ?>')"
                            ...
                            <tr>
                                <td colspan="7">
                                    <div class="ui-empty-state py-5">
                                        ...
                                        <form method="POST" action="<?php echo e(route('owner.backups.store')); ?>" style="display:inline;">
                                            <?php echo csrf_field(); ?>
...
<div class="modal fade" id="customNameModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:16px; border:1px solid rgba(245,158,11,.2);">
            <form method="POST" action="<?php echo e(route('owner.backups.store')); ?>" id="customNameForm">
                <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/sistema-facturacion/resources/views/owner/backups/index.blade.php ENDPATH**/ ?>