...
<head>
    <meta charset="UTF-8">
    <title>e-CF <?php echo e($ecf->encf); ?></title>
    <style>
        @page { margin: 1cm; }
...
<body>
            ...
            <tr>
                <td style="width: 60%; vertical-align: top; padding: 0;">
                    ...
                    <div style="margin-bottom: 4px;">
                        <img src="<?php echo e($pdfLogoUrl); ?>" style="max-width: 70px; max-height: 50px; object-fit: contain;" alt="Logo">
                    </div>
                    <?php endif; ?>
                    <div class="empresa-nombre"><?php echo e(\App\Models\SystemSetting::nombreEmpresaActual()); ?></div>
                    <?php if(!empty($empresa['empresa_rnc'])): ?>
                    <div class="rnc">RNC: <?php echo e($empresa['empresa_rnc']); ?></div>
                    <?php endif; ?>
                    <div class="rnc"><?php echo e($empresa['empresa_direccion'] ?? 'Santo Domingo, R.D.'); ?></div>
                    <div class="rnc">Tel: <?php echo e($empresa['empresa_telefono'] ?? '809-000-0000'); ?></div>
                </td>
                <td style="width: 40%; vertical-align: top; padding: 0;" class="titulo">
                    <span class="tipo">COMPROBANTE FISCAL ELECTRÓNICO</span>
                    <h1>e-CF</h1>
                    <div class="encf-box">
                        <div class="encf"><?php echo e($ecf->encf); ?></div>
                        <div class="small"><?php echo e($ecf->tipo_nombre); ?></div>
                    </div>
                </td>
            </tr>
        </table>
    ...
    <div class="text-center" style="margin-bottom: 10px;">
        <span class="estado <?php echo e($estadoClass); ?>">
            ESTADO: <?php echo e(strtoupper($estadoInfo['label'])); ?>

            <?php if($ecf->track_id_dgii): ?>
                | Track ID: <?php echo e($ecf->track_id_dgii); ?>
    ...
    <table style="margin-bottom: 15px;">
        <tr>
            <td style="width: 50%; padding: 5px; background: #fafafa; vertical-align: top;">
                ...
                <?php if(!empty($empresa['empresa_rnc'])): ?>
                RNC: <?php echo e($empresa['empresa_rnc']); ?>

                <?php endif; ?>
            </td>
            <td style="width: 50%; padding: 5px; background: #fafafa; vertical-align: top;">
                <strong style="font-size: 9px;">FECHA DE EMISIÓN:</strong> <?php echo e($ecf->fecha_emision->format('d/m/Y')); ?><br>
                <strong style="font-size: 9px;">HORA:</strong> <?php echo e($ecf->fecha_emision->format('h:i:s A')); ?><br>
                <?php if($ecf->fecha_aprobacion): ?>
                <strong style="font-size: 9px;">APROBADO DGII:</strong> <?php echo e($ecf->fecha_aprobacion->format('d/m/Y h:i:s A')); ?>
    ...
    <?php if($ecf->venta && $ecf->venta->cliente): ?>
    <table style="margin-bottom: 15px;">
        <tr>
        ...
        <tr>
            <td style="width: 50%; padding: 5px;">
                ...
                <?php echo e($ecf->venta->cliente->nombre ?? 'Consumidor Final'); ?>

            </td>
            <td style="width: 50%; padding: 5px;">
                <strong style="font-size: 8px;">RNC / CÉDULA:</strong>
                <?php echo e($ecf->venta->cliente?->rnc_cedula ?? 'N/A'); ?>
    ...
    <table>
        ...
        <tbody>
            <?php $__currentLoopData = $ecf->venta->detalles->where('tipo_linea', '!=', 'delivery'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <tr>
                <td><?php echo e($i + 1); ?></td>
                <td><?php echo e($d->producto->nombre ?? $d->obra->titulo ?? 'Producto'); ?></td>
                <td class="text-end"><?php echo e($d->cantidad); ?></td>
                <td class="text-end">$<?php echo e(number_format($d->precio_unitario, 2)); ?></td>
                <?php if($ecf->itbis_total > 0): ?>
                <td class="text-end">$<?php echo e(number_format($d->sin_itbis ? 0 : $d->subtotal * (($d->producto->itbis_porcentaje ?? $d->itbis_porcentaje ?? 0) / 100), 2)); ?></td>
                <?php endif; ?>
            ...
            <tr>
                <td colspan="<?php echo e($ecf->itbis_total > 0 ? '6' : '5'); ?>" style="font-size: 8px; color: #0d6efd; padding-left: 20px;">Garantia: <?php echo e($d->producto->garantia_meses); ?> meses</td>
            </tr>
            <?php endif; ?>
    ...
    <table class="totales">
        <tr>
            <td>Monto Gravado</td>
            <td class="text-end">$<?php echo e(number_format($ecf->monto_gravado_total, 2)); ?></td>
        </tr>
        <tr>
            <td>Monto Exento</td>
            <td class="text-end">$<?php echo e(number_format($ecf->monto_exento_total, 2)); ?></td>
        </tr>
        <?php if($ecf->itbis_total > 0): ?>
        <tr>
            <td>ITBIS (18%)</td>
            <td class="text-end">$<?php echo e(number_format($ecf->itbis_total, 2)); ?></td>
        </tr>
        <?php endif; ?>
        <tr class="total-final">
            <td>TOTAL</td>
            <td class="text-end">$<?php echo e(number_format($ecf->monto_total, 2)); ?></td>
        </tr>
    </table>

    <div class="footer">
        <div class="qr-section">
            <img src="<?php echo e($qrUrl); ?>" alt="QR de Consulta"><br>
            <div class="small">Consulte este comprobante en DGII</div>
            <div class="small" style="margin-top: 5px;">Código: <strong><?php echo e($ecf->codigo_seguridad); ?></strong></div>
        </div>
        <div class="info-section">
            ...
            <?php if($ecf->estado !== 'aprobado'): ?>
            <p class="small" style="color: #856404;">
                <strong>VERSIÓN PRELIMINAR</strong> - Este e-CF aún no ha sido aprobado por la DGII.
                <?php if($ecf->mensaje_dgii): ?><br>Motivo: <?php echo e($ecf->mensaje_dgii); ?><?php endif; ?>
            </p>
            <?php else: ?>
            <p class="small" style="color: #155724;">
                <strong>APROBADO POR DGII</strong> - Track ID: <?php echo e($ecf->track_id_dgii); ?><br>
                Firma Digital: <?php echo e(substr($ecf->firma_digital ?? 'N/A', 0, 60)); ?>...
            </p>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div style="clear:both; margin-top:15px; padding-top:10px; border-top:1px solid #ccc;">
        ...
        <div style="margin-bottom:8px;">
            <strong style="font-size:10px; color:#333;"><?php echo e($t['producto']); ?>:</strong>
            <p style="margin:2px 0 0 8px; font-size:9px; color:#333; line-height:1.4;"><?php echo e($t['terminos']); ?></p>
        </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
    <?php endif; ?>
...
<?php /**PATH /var/www/html/sistema-facturacion/resources/views/ventas/ecf-pdf.blade.php ENDPATH**/ ?>