<?php $__env->startSection('title', 'Mi Suscripción'); ?>
...
<div class="row g-4">
    <div class="col-12">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-1">
            ...
            <a href="<?php echo e(route('suscripcion.pagos')); ?>" class="btn btn-outline-primary btn-sm rounded-pill">
                <i class="bi bi-receipt me-1"></i>Historial de Pagos
            </a>
        </div>
        <p class="text-muted small mb-0">Estado y gestión de tu plan en <?php echo e(config('app.name')); ?>.</p>
    </div>
    ...
    <div class="col-lg-8">
        ...
        <div class="card shadow-sm border-0 rounded-4 mb-4">
            <div class="card-body p-4">
                <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-3">
                    <div>
                        <h5 class="fw-bold mb-1"><?php echo e($instance->nombre); ?></h5>
                        <span class="badge bg-light text-dark border rounded-pill">
                            Plan <strong><?php echo e($instance->plan?->nombre ?? '—'); ?></strong>
                        </span>
                    </div>
                    <span class="badge <?php echo e($badge[0]); ?> rounded-pill px-3 py-2 fs-6">
                        <i class="bi <?php echo e(['prueba'=>'bi-rocket-takeoff','activa'=>'bi-check-circle-fill','atrasada'=>'bi-exclamation-triangle-fill','suspendida'=>'bi-lock-fill'][$estado] ?? 'bi-question-circle'); ?> me-1"></i>
                        <?php echo e($badge[1]); ?>
                ...
                <div class="row g-3 text-center">
                    <div class="col-6 col-md-3">
                        <div class="border rounded-3 p-3 bg-light">
                            ...
                            <div class="fw-bold fs-5">RD$ <?php echo e(number_format($instance->precioMensual(), 2)); ?></div>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="border rounded-3 p-3 bg-light">
                            <div class="text-muted small">Próximo Pago</div>
                            <div class="fw-bold fs-5"><?php echo e(optional($instance->proximoPagoEsperado())->format('d/m/Y') ?? '—'); ?></div>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="border rounded-3 p-3 bg-light">
                            <div class="text-muted small">Meses Atrasados</div>
                            <div class="fw-bold fs-5"><?php echo e($instance->mesesAtrasados()); ?></div>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="border rounded-3 p-3 bg-light">
                            <div class="text-muted small">Deuda Estimada</div>
                            <div class="fw-bold fs-5">RD$ <?php echo e(number_format($instance->deudaEstimada(), 2)); ?></div>
                        </div>
                    </div>
                </div>

                <?php if($estado === 'prueba'): ?>
                    <div class="alert alert-primary d-flex align-items-center gap-2 mt-4 mb-0 rounded-4">
                        <i class="bi bi-hourglass-split"></i>
                        <div>
                            Estás en tu <strong>prueba gratuita de <?php echo e($instance->trialDays()); ?> días</strong>.
                            Termina el <strong><?php echo e(optional($instance->trial_ends_at)->format('d/m/Y')); ?></strong>.
                            <?php if($instance->diasPruebaRestantes() > 0): ?>
                                Quedan <strong><?php echo e($instance->diasPruebaRestantes()); ?> día(s)</strong>.
                            <?php else: ?>
                ...
                <?php if($pendiente): ?>
                    <div class="alert alert-warning d-flex align-items-center gap-2 mt-4 mb-0 rounded-4">
                        ...
                        <div>
                            Tienes un pago <strong>pendiente de confirmación</strong> (referencia <code><?php echo e($pendiente->referencia_externa); ?></code>).
        ...
        <div class="card shadow-sm border-0 rounded-4">
            <div class="card-body p-4">
                        ...
                        <div class="col-md-6">
                            <div class="border rounded-3 p-3 h-100 bg-light">
                                <div class="d-flex align-items-center gap-2 mb-1">
                                    ...
                                    <strong><?php echo e($cuenta->banco); ?></strong>
                                </div>
                                <div class="small mb-1"><strong><?php echo e($cuenta->nombre); ?></strong> — <?php echo e($cuenta->tipo_cuenta); ?></div>
                                <div class="small text-muted">Nº <?php echo e($cuenta->numero_cuenta); ?></div>
                                <?php if($cuenta->titular): ?>
                                    <div class="small text-muted">Titular: <?php echo e($cuenta->titular); ?></div>
                                <?php endif; ?>
                            </div>
                        </div>
                ...
                <?php if(! $pendiente): ?>
                <form method="POST" action="<?php echo e(route('suscripcion.pagar')); ?>" class="mt-2">
                    <?php echo csrf_field(); ?>
                    <div class="row g-3">
                        <div class="col-md-5">
                            <label class="form-label">Monto a pagar (RD$)</label>
                            <input type="number" step="0.01" min="1" name="monto" class="form-control"
                                   value="<?php echo e(old('monto', $instance->deudaEstimada() > 0 ? number_format($instance->deudaEstimada(), 2, '.', '') : $instance->precioMensual())); ?>" required>
                        </div>
                        <div class="col-md-7">
                            ...
                            <input type="text" name="referencia_externa" class="form-control" maxlength="120"
                                   placeholder="Nº de referencia / comprobante" value="<?php echo e(old('referencia_externa')); ?>" required>
                        </div>
                    </div>
                    <div class="mt-3">
    ...
    <div class="col-lg-4">
        <div class="card shadow-sm border-0 rounded-4">
            <div class="card-body p-4">
                ...
                <?php $modulos = $instance->plan?->modulosPermitidos() ?? []; ?>
                <?php if($modulos === []): ?>
                ...
                <?php else: ?>
                    <div class="d-flex flex-wrap gap-2">
                        <?php $__currentLoopData = $modulos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <span class="badge bg-light text-dark border rounded-pill"><?php echo e(ucfirst($m)); ?></span>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                <?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/sistema-facturacion/resources/views/suscripcion/index.blade.php ENDPATH**/ ?>