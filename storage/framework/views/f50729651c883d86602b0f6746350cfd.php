<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Inventario al <?php echo e(now()->format('d/m/Y')); ?></title>
<style>body{font-family:DejaVu Sans,sans-serif;font-size:8px;}table{width:100%;border-collapse:collapse;margin-top:10px;}th,td{border:1px solid #ccc;padding:3px 5px;text-align:left;}th{background:#f0f0f0;font-weight:700;text-transform:uppercase;font-size:7px;}td.text-end{text-align:right;}.totals{background:#f8f8f8;font-weight:700;}h2{margin:0;color:#333;}.meta{color:#666;margin:5px 0;}</style>
</head><body>
<?php $empresa = \App\Models\SystemSetting::allCached(); ?>
<div style="text-align:center;margin-bottom:8px;">
    <?php if($pdfLogoUrl): ?>
    <img src="<?php echo e($pdfLogoUrl); ?>" style="max-width: 80px; max-height: 60px; object-fit: contain; margin-bottom: 5px;" alt="Logo">
    <?php endif; ?>
    <strong style="font-size:11px;"><?php echo e(\App\Models\SystemSetting::nombreEmpresaActual()); ?></strong><br>
    <span style="font-size:7px;color:#666;">RNC: <?php echo e($empresa['empresa_rnc'] ?? 'N/A'); ?></span>
</div>
<h2>Reporte de Inventario</h2>
<p class="meta">Generado: <?php echo e(now()->format('d/m/Y H:i')); ?> &middot; <?php echo e($totalProductos); ?> producto(s)</p>
<table><thead><tr>
<th>Código</th><th>Producto</th><th class="text-end">Stock</th><th class="text-end">Mínimo</th><th class="text-end">Costo</th><th class="text-end">Precio</th><th class="text-end">Valor Inv.</th>
</tr></thead><tbody>
<?php $__currentLoopData = $productos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
<tr>
<td><?php echo e($p->codigo_barras ?? $p->referencia ?? '-'); ?></td>
<td><?php echo e($p->nombre); ?></td>
<td class="text-end"><?php echo e($p->stock); ?></td>
<td class="text-end"><?php echo e($p->stock_minimo ?? 0); ?></td>
<td class="text-end">RD$ <?php echo e(number_format($p->precio_compra ?? 0, 2)); ?></td>
<td class="text-end">RD$ <?php echo e(number_format($p->precio ?? 0, 2)); ?></td>
<td class="text-end">RD$ <?php echo e(number_format($p->stock * ($p->precio_compra ?? 0), 2)); ?></td>
</tr>
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
</tbody>
<tfoot>
<tr class="totals">
<td colspan="6" class="text-end">VALOR TOTAL INVENTARIO</td>
<td class="text-end">RD$ <?php echo e(number_format($totalValorInventario, 2)); ?></td>
</tr>
</tfoot></table>
</body></html><?php /**PATH /var/www/html/sistema-facturacion/resources/views/reportes/stock-pdf.blade.php ENDPATH**/ ?>