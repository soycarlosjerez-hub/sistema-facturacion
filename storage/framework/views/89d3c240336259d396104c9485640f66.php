    ...
    <div class="row g-3 mb-4">
                    ...
                    <table class="table table-hover align-middle mb-0 precios-table" id="tablaPrecios">
                        ...
                        <tbody>
                            ...
                            <?php
                                ...
                                $marginSign = $margen > 0 ? '+' : '';
                            ?>
                            <tr data-producto-id="<?php echo e($p->id); ?>" class="<?php echo e($item ? '' : 'd-none producto-no-lista'); ?>">
                                <td class="ps-4"><span class="badge bg-light text-muted rounded-pill"><?php echo e($p->codigo_barras ?? '&mdash;'); ?></span></td>
                                <td class="fw-bold small"><?php echo e($p->nombre); ?></td>
                                <td class="text-end text-muted">RD$ <?php echo e(number_format($costo, 2)); ?></td>
                                <td class="text-end <?php echo e($marginClass); ?>">
                                    <?php if($costo > 0 && $precioLista > 0): ?>
                                        <?php echo e($marginSign); ?><?php echo e(number_format($margen, 1)); ?>%
                                    <?php else: ?>
                                        &mdash;
                                    <?php endif; ?>
                                </td>
                                <td class="text-end text-muted">RD$ <?php echo e(number_format($p->precio, 2)); ?></td>
                                <td class="text-end">
                                    <input type="number" step="0.01" min="0" class="form-control form-control-sm text-end precio-lista"
                                           style="width:130px;display:inline-block;"
                                           value="<?php echo e($item ? number_format($item->precio, 2, '.', '') : ''); ?>"
                                           placeholder="0.00"
                                           data-producto-id="<?php echo e($p->id); ?>">
                                </td>
        ...
        <div class="col-md-4">
            <div class="ui-card mb-3" style="--delay:.15s">
                ...
                <div class="card-body">
                    <div class="small">
                        ...
                        <div class="mb-2"><span class="text-muted">Vigencia:</span> <?php echo e($listaPrecio->vigencia_desde->format('d/m/Y')); ?> - <?php echo e($listaPrecio->vigencia_hasta?->format('d/m/Y') ?? 'Indefinida'); ?></div>
                        <?php endif; ?>
                    </div>
                    <hr>
                    <form id="listaPrecioForm" action="<?php echo e(route('listas-precio.update', $listaPrecio)); ?>" method="POST">
                        <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
                        <div class="mb-2">
                            <label class="ui-label">Nombre</label>
                            <input type="text" name="nombre" class="ui-input" value="<?php echo e($listaPrecio->nombre); ?>" required>
                        </div>
                        <div class="mb-2">
                            <label class="ui-label">C&oacute;digo</label>
                            <input type="text" name="codigo" class="ui-input" value="<?php echo e($listaPrecio->codigo); ?>" required>
                        </div>
                        <div class="mb-2">
                            <label class="ui-label">Descripci&oacute;n</label>
                            <textarea name="descripcion" class="ui-textarea" rows="2"><?php echo e($listaPrecio->descripcion); ?></textarea>
                        </div>
                        <div class="row g-2 mb-2">
                            <div class="col-6">
                                <label class="ui-label">Vigencia desde</label>
                                <input type="date" name="vigencia_desde" class="ui-input" value="<?php echo e($listaPrecio->vigencia_desde?->format('Y-m-d')); ?>">
                            </div>
                            <div class="col-6">
                                <label class="ui-label">Vigencia hasta</label>
                                <input type="date" name="vigencia_hasta" class="ui-input" value="<?php echo e($listaPrecio->vigencia_hasta?->format('Y-m-d')); ?>">
                            </div>
                        </div>
                        <div class="form-check form-switch mb-3">
                            <input class="form-check-input" type="checkbox" role="switch" id="activa" name="activa" value="1" <?php echo e($listaPrecio->activa ? 'checked' : ''); ?>>
            ...
            <div class="ui-card" style="--delay:.2s">
                ...
                <div class="card-body">
                    <a href="<?php echo e(route('listas-precio.impacto', $listaPrecio)); ?>" class="btn btn-outline-warning w-100 rounded-pill btn-sm mb-2">
                        <i class="bi bi-graph-up me-1"></i>Impacto de Precios
                    </a>
                    <a href="<?php echo e(route('listas-precio.logs', $listaPrecio)); ?>" class="btn btn-outline-secondary w-100 rounded-pill btn-sm mb-2">
                        <i class="bi bi-clock-history me-1"></i>Historial de Cambios
                    </a>
                    <form action="<?php echo e(route('listas-precio.duplicar', $listaPrecio)); ?>" method="POST" class="mb-2">
                        <?php echo csrf_field(); ?>
                        <button class="btn btn-outline-info w-100 rounded-pill btn-sm">
                            <i class="bi bi-copy me-1"></i>Duplicar lista
                        </button>
                    </form>
                    <form action="<?php echo e(route('listas-precio.destroy', $listaPrecio)); ?>" method="POST" class="d-inline">
                        <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                        <button type="button" class="ui-action ui-action-delete w-100 rounded-pill btn-sm" onclick="UI.confirm.delete('<?php echo e(route('listas-precio.destroy', $listaPrecio)); ?>', '<?php echo e(addslashes($listaPrecio->nombre)); ?>')">
    ...
    <div id="stickySaveBar" class="ui-sticky-bar">
        <div class="ui-sticky-bar-inner flex-wrap gap-2">
            <div class="d-flex align-items-center gap-2">
                ...
                <span class="fw-semibold d-none d-sm-inline">Editando: <?php echo e($listaPrecio->nombre); ?></span>
...
<form id="formActualizarPrecios" action="<?php echo e(route('listas-precio.actualizar-precios', $listaPrecio)); ?>" method="POST">
    <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/sistema-facturacion/resources/views/listas-precio/edit.blade.php ENDPATH**/ ?>