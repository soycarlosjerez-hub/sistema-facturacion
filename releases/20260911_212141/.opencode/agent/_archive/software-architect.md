...
- Para análisis de negocio y requisitos: consulta al agente `business-analyst`

## Trigger Keywords

"arquitectura", "diseño", "patron", "escala", "refactor", "estructura", "modelo de datos", "relaciones", "decision tecnica", "trade-off", "DDD", "CQRS", "event sourcing", "arquitectura hexagonal", "clean architecture"

## Reglas de Optimización de Recursos

1. **Evitar duplicación de conocimiento** — Los patterns de TenantScope, flash messages y UI premium ya están documentados en los agentes especializados
2. **Lazy loading de知识** — Cargar knowledge específica por necesidad, no toda de una vez
3. **Caching de decisiones arquitectónicas** — Decisiones de diseño una vez validadas no se re-discuten
4. **Frontend-Backend boundary clear** — Frontend foca en UI, backend en lógica, database en queries
5. **Testing como capa independiente** — Tests no re-definen arquitectura, solo la validan