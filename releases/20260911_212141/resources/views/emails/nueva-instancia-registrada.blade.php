...
<div class="container">
    ...
    <div class="content">
        ...
        <div class="details">
            <p><strong>Negocio:</strong> {{ $instance->nombre }}</p>
            <p><strong>RNC:</strong> {{ $instance->rnc ?? '—' }}</p>
            <p><strong>Tel&eacute;fono:</strong> {{ $instance->telefono ?? '—' }}</p>
            <p><strong>Direcci&oacute;n:</strong> {{ $instance->direccion ?? '—' }}</p>
            <p><strong>Tipo de negocio:</strong> <span class="badge">{{ $instance->businessType?->nombre ?? '—' }}</span></p>
            <p><strong>Plan:</strong> <span class="badge">{{ $instance->plan?->nombre ?? '—' }}</span> &mdash; RD$ {{ number_format($instance->plan?->precio_mensual ?? 0, 2) }}/mes</p>
            <p><strong>Admin registrado:</strong> {{ $adminName }} ({{ $adminEmail }})</p>
        ...
        <p style="text-align: center; margin: 25px 0 15px 0;">
            <a href="{{ $instanceUrl }}" class="btn">Ver Instancia</a>
        </p>
