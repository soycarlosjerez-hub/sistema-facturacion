        ...
        @else
            @if($pendingApprovals > 0)
                <div class="d-flex align-items-center gap-3 p-3 rounded-3 mb-2" style="background:rgba(245,158,11,.06);border:1px solid rgba(245,158,11,.12);">
                    ...
                    <div class="flex-grow-1" style="min-width:0;">
                        <strong class="small">{{ $pendingApprovals }} solicitud(ones) de registro</strong>
                        <div class="text-muted" style="font-size:.72rem;">Esperando aprobación del owner</div>
                    </div>
                    <a href="{{ route('owner.solicitudes.index') }}" class="ui-btn ui-btn-solid ui-btn-sm rounded-pill flex-shrink-0" style="background:linear-gradient(135deg,#f59e0b,#d97706);border-color:#d97706;">
            ...
            @if($bloqueadas > 0)
                <div class="d-flex align-items-center gap-3 p-3 rounded-3 mb-2" style="background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.12);">
                    ...
                    <div class="flex-grow-1" style="min-width:0;">
                        <strong class="small">{{ $bloqueadas }} instancia(s) bloqueada(s)</strong>
                        <div class="text-muted" style="font-size:.72rem;">Por impago o motivo manual</div>
                    </div>
                    <a href="{{ route('owner.instances.index') }}" class="ui-btn ui-btn-solid ui-btn-sm rounded-pill flex-shrink-0" style="background:linear-gradient(135deg,#ef4444,#dc2626);border-color:#dc2626;">
            ...
            @if($churnRiskCount > 0)
                <div class="d-flex align-items-center gap-3 p-3 rounded-3 mb-2" style="background:rgba(245,158,11,.06);border:1px solid rgba(245,158,11,.12);">
                    ...
                    <div class="flex-grow-1" style="min-width:0;">
                        <strong class="small">{{ $churnRiskCount }} en riesgo de churn</strong>
                        <div class="text-muted" style="font-size:.72rem;">Prueba perdida, sin pago</div>
                    </div>
                    <a href="{{ route('owner.instances.index') }}" class="ui-btn ui-btn-ghost ui-btn-sm rounded-pill flex-shrink-0">
                        Contactar
                    </a>
                </div>
            @endif
            ...
            @if($erroresCriticos24h > 0)
                <div class="d-flex align-items-center gap-3 p-3 rounded-3 mb-2" style="background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.12);">
                    ...
                    <div class="flex-grow-1" style="min-width:0;">
                        <strong class="small">{{ $erroresCriticos24h }} error(es) crítico(s)</strong>
                        <div class="text-muted" style="font-size:.72rem;">Últimas 24 horas</div>
                    </div>
                    <a href="{{ route('owner.errors.index') }}" class="ui-btn ui-btn-ghost ui-btn-sm rounded-pill flex-shrink-0">
                        Ver
                    </a>
                </div>
            @endif

            @if($instanciasConAtraso->count() > 0)
                <div class="d-flex align-items-center gap-3 p-3 rounded-3" style="background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.12);">
                    ...
                    <div class="flex-grow-1" style="min-width:0;">
                        <strong class="small">{{ $instanciasConAtraso->count() }} con pago atrasado</strong>
                        <div class="text-muted" style="font-size:.72rem;">Deuda total: {{ $systemMoneda ?? 'RD$' }} {{ number_format($deudaTotal, 2) }}</div>
                    </div>
                    <a href="{{ route('owner.instances.index') }}" class="ui-btn ui-btn-solid ui-btn-sm rounded-pill flex-shrink-0" style="background:linear-gradient(135deg,#10b981,#059669);border-color:#059669;color:#fff;">
