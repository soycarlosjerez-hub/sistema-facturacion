...
class Sidebar
{
    public static function menu(): array
    {
        ...
        if (
            ...
            if ($hasConf('plantillas') && $can('plantillas.view')) {
                $items[] = ['route' => 'plantillas.index', 'icon' => 'bi-file-earmark-richtext', 'label' => 'Plantillas de Factura', 'is_route' => 'plantillas.*', 'exact_route' => 'plantillas.index'];
            }
            if ($hasConf('configuracion-general') && $can('configuracion.view')) {
                ...
                $isOwnerRoot = false;
                try {
                    $isOwnerRoot = $user->hasRole('owner') || $user->hasRole('root');
                } catch (\Throwable $e) {
                    report($e);
                }
                if ($isOwnerRoot) {
            ...
            $isOwnerRoot2 = false;
            try {
                $isOwnerRoot2 = $user->hasRole('owner') || $user->hasRole('root');
            } catch (\Throwable $e) {
                report($e);
            }
