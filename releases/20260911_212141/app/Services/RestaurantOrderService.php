...
class RestaurantOrderService
{
    ...
    public function abrirCaja(int $cajaId, float $montoInicial): array
    {
        ...
        if (! $caja->activo) {
            return ['error' => 'Esta caja está inactiva', 'code' => 422];
        }
        if ($caja->estado === 'abierta') {
            return ['error' => 'La caja ya está abierta', 'code' => 422];
        }
        ...
        foreach ($ventas as $v) {
            foreach ($v->pagos as $p) {
                switch ($p->metodo_pago) {
                    case 'efectivo': $totalEfectivo += $p->monto;
                        break;
                    case 'tarjeta': $totalTarjeta += $p->monto;
                        break;
                    case 'transferencia': $totalTransferencia += $p->monto;
                        break;
                }
            }
        }
