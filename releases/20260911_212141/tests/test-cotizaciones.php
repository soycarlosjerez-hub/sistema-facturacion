...
test('Cotizacion->user() funciona', function () use (&$cotizacionTest) {
    ...
    if (! $cotizacionTest->user) {
        throw new Exception('Usuario no cargado');
    }
...
test('Cliente->cotizaciones() (relación inversa) funciona', function () use ($cli) {
    ...
    if ($count === 0) {
        throw new Exception('No se cargaron cotizaciones del cliente');
    }
...
test('Accessor dias_validez funciona', function () use (&$cotizacionTest) {
    ...
    if (! is_int($dias)) {
        throw new Exception('dias_validez no es entero');
    }
...
test('Permisos asignados a rol admin', function () {
    ...
    if ($count < 7) {
        throw new Exception("Admin tiene $count, esperaba al menos 7");
    }
...
test('Conversión: crea venta con totales correctos', function () use (&$cotizacionTest, $user, $almacen) {
    ...
    try {
        ...
        if (! $tipoVenta) {
            throw new Exception('No hay tipos de venta');
        }
...
test('Rutas de cotizaciones registradas', function () {
    $routes = collect(Illuminate\Support\Facades\Route::getRoutes())
        ...
        ->count();
    if ($routes < 10) {
        throw new Exception("Solo $routes rutas, esperaba al menos 10");
    }
