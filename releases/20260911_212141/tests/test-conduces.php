...
test('Migraciones: tabla conduces existe', function () {
    if (! \Schema::hasTable('conduces')) {
        throw new \Exception('Tabla conduces no existe');
    }
    if (! \Schema::hasTable('conduce_items')) {
        throw new \Exception('Tabla conduce_items no existe');
    }
});

test('Migraciones: columna deleted_at (soft delete) en conduces', function () {
    if (! \Schema::hasColumn('conduces', 'deleted_at')) {
        throw new \Exception('Falta deleted_at en conduces');
    }
});
...
test('Migraciones: columnas requeridas en conduces', function () {
    ...
    foreach ($cols as $c) {
        if (! \Schema::hasColumn('conduces', $c)) {
            throw new \Exception("Falta columna $c");
        }
    }
});
...
test('Migraciones: columnas en conduce_items', function () {
    ...
    foreach ($cols as $c) {
        if (! \Schema::hasColumn('conduce_items', $c)) {
            throw new \Exception("Falta columna $c");
        }
    }
});
...
test('Modelo: constante ESTADOS tiene los 5 estados', function () {
    ...
    foreach ($esperados as $e) {
        if (! isset(Conduce::ESTADOS[$e])) {
            throw new \Exception("Falta estado $e");
        }
    }
});
...
test('Modelo: scopes accesibles', function () {
    ...
    foreach ($metodos as $m) {
        if (! method_exists(Conduce::class, $m)) {
            throw new \Exception("Falta $m");
        }
    }
});
...
test('Modelo: accessors de estado', function () {
    ...
    if ($c->estado_label !== 'Borrador') {
        throw new \Exception('estado_label mal: '.$c->estado_label);
    }
    if ($c->estado_color !== 'secondary') {
        throw new \Exception('estado_color mal: '.$c->estado_color);
    }
    if ($c->estado_icon !== 'pencil-square') {
        throw new \Exception('estado_icon mal: '.$c->estado_icon);
    }
});

test('Modelo: accessors booleanos (puede_entregarse, esta_vencido)', function () {
    $c = new Conduce(['estado' => 'en_transito']);
    if ($c->puede_entregarse !== true) {
        throw new \Exception('puede_entregarse debe ser true en tránsito');
    }

    $c2 = new Conduce(['estado' => 'entregado']);
    if ($c2->puede_entregarse !== false) {
        throw new \Exception('puede_entregarse debe ser false cuando ya entregado');
    }
});
...
test('CRUD: crear conduce con items', function () use ($admin, $cliente, $producto, &$conduceTest) {
    ...
    if ($conduce->items->count() !== 2) {
        throw new \Exception('Items no guardados');
    }
    if ($conduce->total_items !== 15) {
        throw new \Exception("total_items={$conduce->total_items}, esperado 15");
    }
    if ((float) $conduce->peso_total !== 1.5) {
        throw new \Exception("peso_total={$conduce->peso_total}");
    }
...
test('CRUD: leer conduce con relaciones', function () use (&$conduceTest) {
    if (! $conduceTest) {
        throw new \Exception('No hay conduce');
    }
    $c = Conduce::with(['cliente', 'user', 'items'])->find($conduceTest->id);
    if (! $c->cliente) {
        throw new \Exception('Falta relación cliente');
    }
    if (! $c->user) {
        throw new \Exception('Falta relación user');
    }
    if ($c->items->count() !== 2) {
        throw new \Exception('Items no cargados');
    }
});
...
test('CRUD: actualizar conduce', function () use (&$conduceTest) {
    ...
    if (! str_contains($fresh->observaciones, 'Test update')) {
        throw new \Exception('Update no persistió');
    }
});
...
test('Estados: cambiar borrador → en_transito', function () use (&$conduceTest) {
    ...
    $fresh = Conduce::find($conduceTest->id);
    if ($fresh->estado !== 'en_transito') {
        throw new \Exception("Estado no cambió: {$fresh->estado}");
    }
});
...
test('Entrega: marcar como entregado con cantidades', function () use (&$conduceTest) {
    ...
    if (! $ok) {
        throw new \Exception('marcarEntregado devolvió false');
    }
    $fresh = Conduce::with('items')->find($conduceTest->id);
    if ($fresh->estado !== 'entregado') {
        throw new \Exception("Estado no es entregado: {$fresh->estado}");
    }
    if ($fresh->recibido_por !== 'Receptor Test') {
        throw new \Exception('Recibido por no guardado');
    }
    if (! $fresh->fecha_recibido) {
        throw new \Exception('fecha_recibido no guardada');
    }
...
test('Items: accessors de ConduceItem', function () use (&$conduceTest) {
    ...
    if ($item->entregado_completo !== true) {
        throw new \Exception('entregado_completo debe ser true');
    }
    if ($item->porcentaje_entregado != 100) {
        throw new \Exception("porcentaje_entregado={$item->porcentaje_entregado}");
    }
});
...
test('Rutas: 13 rutas de conduces registradas', function () {
    ...
    foreach ($expected as $e) {
        if (! in_array($e, $names, true)) {
            throw new \Exception("Falta ruta $e");
        }
    }
});
...
test('Rutas: constraints where([0-9]+) en parámetros numéricos', function () {
    ...
    foreach ($routes as $r) {
        if (str_contains($r->uri(), 'conduces/{conduce}/ticket') && $r->getName() === 'conduces.ticket') {
            $found = true;
            if (! $r->wheres) {
                throw new \Exception('Sin constraints where');
            }
            break;
        }
    }
    if (! $found) {
        throw new \Exception('Ruta conduces.ticket no encontrada');
    }
});
...
test('Vistas: archivos existen', function () {
    ...
    foreach ($vistas as $v) {
        if (! view()->exists($v)) {
            throw new \Exception("Falta vista $v");
        }
    }
});
...
test('Vistas: ticket usa 80/58mm', function () {
    ...
    if (! $c) {
        throw new \Exception('No hay conduce entregado');
    }

    $html80 = view('conduces.ticket', ['conduce' => $c, 'paper' => 80, 'empresa' => (object) []])->render();
    if (! str_contains($html80, '80mm')) {
        throw new \Exception('No renderiza para 80mm');
    }
    if (! str_contains($html80, $c->numero)) {
        throw new \Exception('No muestra número');
    }

    $html58 = view('conduces.ticket', ['conduce' => $c, 'paper' => 58, 'empresa' => (object) []])->render();
    if (! str_contains($html58, '58mm')) {
        throw new \Exception('No renderiza para 58mm');
    }
});
...
test('Vistas: PDF renderiza', function () {
    ...
    if (! str_contains($html, 'CONDUCE')) {
        throw new \Exception('PDF no tiene header');
    }
    if (! str_contains($html, $c->numero)) {
        throw new \Exception('PDF no muestra número');
    }
    if (! str_contains($html, 'Despachado por')) {
        throw new \Exception('PDF sin firma de despachado');
    }
    if (! str_contains($html, 'Recibido por')) {
        throw new \Exception('PDF sin firma de receptor');
    }
});
...
test('Controller: ConduceController tiene 13 métodos públicos', function () {
    ...
    foreach ($esperados as $m) {
        if (! in_array($m, $nombres)) {
            throw new \Exception("Falta método $m. Disponibles: ".implode(',', $nombres));
        }
    }
});
...
test('Permisos: admin tiene todos los conduces.*', function () {
    ...
    foreach ($conducesPerms as $p) {
        if (! $adminRole->hasPermissionTo($p)) {
            throw new \Exception("Admin no tiene $p");
        }
    }
});
...
test('Permisos: vendedor tiene view, create, edit, print, deliver', function () {
    ...
    foreach ($esperados as $p) {
        if (! $vendedor->hasPermissionTo($p)) {
            throw new \Exception("Vendedor no tiene $p");
        }
    }
...
test('SoftDelete: eliminar y restaurar', function () use (&$conduceTest) {
    ...
    $conduceTest->delete();
    if (Conduce::find($id)) {
        throw new \Exception('Soft delete no funcionó');
    }
    if (! Conduce::onlyTrashed()->find($id)) {
        throw new \Exception('onlyTrashed no lo encuentra');
    }

    Conduce::onlyTrashed()->find($id)->restore();
    $restored = Conduce::find($id);
    if (! $restored) {
        throw new \Exception('Restore no funcionó');
    }
    if ($restored->estado !== 'entregado') {
        throw new \Exception("Estado perdido: {$restored->estado}");
    }
});
...
test('Snapshot: items preservan datos del producto al crear', function () {
    ...
    if (! $c) {
        throw new \Exception('Sin items con producto');
    }
    $item = $c->items->first();
    if (! $item->nombre) {
        throw new \Exception('Falta snapshot nombre');
    }
    if (! $item->codigo) {
        throw new \Exception('Falta snapshot codigo');
    }
});
...
test('Búsqueda: scope buscar() por número/observaciones', function () {
    $c = Conduce::first();
    if (! $c) {
        throw new \Exception('Sin conduces');
    }
    $busquedaNumero = Conduce::buscar($c->numero)->count();
    if ($busquedaNumero < 1) {
        throw new \Exception("Buscar por número no encontró: {$c->numero}");
    }
});
...
test('Cliente: relación conduces()', function () use ($cliente) {
    ...
    if ($count < 1) {
        throw new \Exception("Cliente no tiene conduces asociados (count={$count})");
    }
});
