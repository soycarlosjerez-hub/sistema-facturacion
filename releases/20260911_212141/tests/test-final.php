...
function test_ok($name, $callback, $section = 'General')
{
    ...
    if (! isset($sections[$section])) {
        $sections[$section] = ['pass' => 0, 'fail' => 0, 'results' => []];
    }
...
if (! $cli) {
    $cli = App\Models\Cliente::factory()->create();
}
if (! $prod) {
    $prod = App\Models\Producto::factory()->create();
}
if (! $user) {
    echo 'No hay usuario. Abortando.'.PHP_EOL;
    exit(1);
}
...
test_ok('Relaciones Eloquent', function () use ($cot) {
    ...
    if (! $cot->cliente) {
        throw new \Exception('Cliente no cargado');
    }
    if (! $cot->user) {
        throw new \Exception('User no cargado');
    }
...
test_ok('Scopes (activas/vencidas)', function () {
    $count = App\Models\Cotizacion::activas()->count();
    if ($count === 0) {
        throw new \Exception('No hay activas');
    }
...
test_ok('Permisos configurados', function () {
    ...
    if ($perms < 6) {
        throw new \Exception("Solo $perms permisos");
    }
...
test_ok('Rutas registradas (11)', function () {
    ...
    if ($count < 11) {
        throw new \Exception("Solo $count rutas");
    }
...
test_ok('CotizacionEmailService existe', function () {
    if (! class_exists('App\Services\CotizacionEmailService')) {
...
test_ok('CotizacionEnviadaMail existe', function () {
    if (! class_exists('App\Mail\CotizacionEnviadaMail')) {
        throw new \Exception('No existe');
    }
...
test_ok('Servicio envia email correctamente', function () use ($cot) {
    ...
    if (! $r['success']) {
        throw new \Exception($r['error'] ?? 'Fallo');
    }
...
test_ok('Email a cliente sin email falla', function () use ($user) {
    ...
    if ($r['success']) {
        throw new \Exception('Deberia fallar');
    }
...
test_ok('Vista HTML del email se renderiza', function () use ($cot) {
    ...
    if (strlen($html) < 1000) {
        throw new \Exception('HTML muy corto');
    }
    if (! str_contains($html, $cot->numero)) {
        throw new \Exception('Falta numero');
    }
...
test_ok('Vista texto del email se renderiza', function () use ($cot) {
    ...
    if (! str_contains($txt, $cot->numero)) {
        throw new \Exception('Falta numero');
    }
...
test_ok('PrintService existe', function () {
    if (! class_exists('App\Services\PrintService')) {
        throw new \Exception('No existe');
    }
...
test_ok('Ticket 80mm se genera', function () use ($cot) {
    ...
    if (strlen($ticket) < 500) {
        throw new \Exception('Muy corto');
    }
    if (! str_contains($ticket, $cot->numero)) {
        throw new \Exception('Falta numero');
    }
...
test_ok('ESC/POS genera comandos validos', function () use ($cot) {
    ...
    if (! str_starts_with($escpos, "\x1B\x40")) {
        throw new \Exception('No inicia con ESC @');
    }
    if (! str_ends_with($escpos, "\x1D\x56\x00")) {
        throw new \Exception('No termina con corte');
    }
...
test_ok('Rutas ticket registradas', function () {
    ...
    if (! $r1 || ! $r2) {
        throw new \Exception('Faltan rutas');
    }
...
test_ok('a11y.js existe', function () {
    if (! file_exists(public_path('js/a11y.js'))) {
        throw new \Exception('No existe');
    }
...
test_ok('a11y.css existe', function () {
    if (! file_exists(public_path('css/a11y.css'))) {
        throw new \Exception('No existe');
    }
...
test_ok('Layout tiene skip link', function () {
    ...
    if (! str_contains($layout, 'skip-to-main')) {
        throw new \Exception('No skip link');
    }
...
test_ok('Layout incluye a11y assets', function () {
    $layout = file_get_contents(resource_path('views/layouts/app.blade.php'));
    if (! str_contains($layout, "asset('js/a11y.js')")) {
        throw new \Exception('No JS');
    }
    if (! str_contains($layout, "asset('css/a11y.css')")) {
        throw new \Exception('No CSS');
    }
...
test_ok('POS search tiene ARIA', function () {
    ...
    if (! str_contains($pos, 'aria-label="Buscar producto')) {
        throw new \Exception('No aria-label');
    }
    if (! str_contains($pos, 'aria-controls=')) {
        throw new \Exception('No aria-controls');
    }
    if (! str_contains($pos, 'role="search"')) {
        throw new \Exception('No role=search');
    }
    if (! str_contains($pos, 'role="listbox"')) {
        throw new \Exception('No role=listbox');
    }
...
test_ok('Modal email tiene aria-labelledby', function () {
    ...
    if (! str_contains($show, 'aria-labelledby="modalEnviarEmailLabel"')) {
...
test_ok('Email template usa ARIA', function () {
    ...
    if (! str_contains($email, 'role="table"')) {
        throw new \Exception('No role=table');
    }
...
test_ok('CSS tiene touch target 44px', function () {
    $css = file_get_contents(public_path('css/a11y.css'));
    if (! str_contains($css, 'min-height: 44px')) {
        throw new \Exception('No');
    }
...
test_ok('JS tiene focus trap', function () {
    ...
    if (! str_contains($js, 'trapFocus')) {
        throw new \Exception('No');
    }
    if (! str_contains($js, 'announce')) {
        throw new \Exception('No announce');
    }
...
test_ok('Calculo: descuento monto', function () {
    ...
    $final = $subtotal - $desc;
    if ($final !== 170) {
        throw new \Exception('Esperado 170');
    }
...
test_ok('Calculo: descuento porcentaje', function () {
    ...
    $final = $subtotal - $desc;
    if ($final !== 180) {
        throw new \Exception('Esperado 180');
    }
...
test_ok('POS tiene input descuento', function () {
    $pos = file_get_contents(resource_path('views/ventas/create.blade.php'));
    if (! str_contains($pos, 'class="discount-input"')) {
...
test_ok('POS tiene toggle tipo descuento', function () {
    $pos = file_get_contents(resource_path('views/ventas/create.blade.php'));
    if (! str_contains($pos, 'discount-toggle')) {
        throw new \Exception('No toggle');
    }
    if (! str_contains($pos, "'toggle-discount-type'")) {
...
test_ok('POS envia descuento[] y descuento_tipo[]', function () {
    $pos = file_get_contents(resource_path('views/ventas/create.blade.php'));
    if (! str_contains($pos, 'name="descuento[]"')) {
        throw new \Exception('No descuento[]');
    }
    if (! str_contains($pos, 'name="descuento_tipo[]"')) {
        throw new \Exception('No descuento_tipo[]');
    }
...
test_ok('VentaController valida descuentos', function () {
    ...
    if (! str_contains($controller, "'descuento' => 'nullable|array'")) {
        throw new \Exception('No descuento');
    }
    if (! str_contains($controller, "'descuento_tipo.*' => 'in:monto,porcentaje'")) {
        throw new \Exception('No tipo');
    }
...
test_ok('Calculo: ITBIS sobre subtotal con descuento', function () {
    ...
    if (abs($total - 200.6) > 0.01) {
        throw new \Exception('Esperado ~200.6');
    }
